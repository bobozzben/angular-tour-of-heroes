...
export class HeroDetailComponent implements OnInit {
  ...
  getProduct(): void {
    ...
    // paramMap 是一個從 URL 中提取的路由引數值的字典。 "id" 對應的值就是要獲取的英雄的 id。
    // 路由引數總會是字串。 JavaScript 的 Number 函式會把字串轉換成數字，英雄的 id 就是數字型別。
    const prono = this.route.snapshot.paramMap.get('prono')!;
    if (prono) {
      this.heroService.log( "取得 " + prono );
      this.heroService.getHeroUsedParam(prono)
        .subscribe(hero => this.hero = hero);
    }
  }
  ...
  save(): void {
    if (this.hero) {
      //  this.heroService.log(`updated hero id=${this.hero.id} name=${this.hero.name}`);
